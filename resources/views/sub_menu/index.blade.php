    ...
    <div id="kt_app_toolbar_container" class="container-fluid d-flex align-items-stretch">
        <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="<?php echo url('sub-menu/add/');?>" class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold"><i class="ki-outline ki-plus fs-2"></i>Add</a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="card-body pt-4 pb-0">
        <div class="d-row row-wrap row-sm-nowrap pb-4">
            <form id="search_from" name="search_from" class="form fv-plugins-bootstrap5 fv-plugins-framework">
                <div class="row">
                    <div class="col-md-4">
                        <label class="fs-6 fw-semibold mb-1 ms-1">Main Menu</label>
                         <select id="module_id" name="module_id" class="form-select form-select-solid form-select" aria-label="Select" data-control="select2" data-placeholder="Select">
                            <option></option>
                    ...
                    <div class="col-md-4 fv-row">
                        <label class="fs-6 fw-semibold mb-1 ms-1">Sub Menu Name</label>
                        <input type="text" id="sub_module_short_name" name="sub_module_short_name" class="form-control form-control-solid" />
                        <span class="error" id="sub_module_short_name_error"></span>
                    </div>
                </div>
                <hr>
                <div>
                ...
                <tbody class="text-gray-600 fw-semibold" id="table-content">
                   @foreach($lists as $key => $list)
                    <tr>
                        ...
                        <td>{{$list->sub_module_short_name}}</td>
                        <td>{{$list->sequence}}</td>
                        <td>
                        ...
                        <td>
                            <a href="<?php echo url('sub-menu/add/'.$list->id);?>" class="btn btn-icon btn-active-light-info w-30px h-30px me-3"><i class="ki-outline ki-pencil text-info fs-3"></i></a>
                            <a href="<?php echo url('sub-menu/view/'.$list->id);?>" class="btn btn-icon btn-active-light-primary w-30px h-30px me-3"><i class="ki-outline ki-eye text-primary fs-3"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
<script src="{{ url('public/validation/sub_menu.js') }}"></script>
@endsection