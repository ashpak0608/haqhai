    ...
    <div id="kt_app_toolbar_container" class="container-fluid d-flex align-items-stretch">
        <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="<?php echo url('district/add/');?>" class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold"><i class="ki-outline ki-plus fs-2"></i>Add</a>
            </div>
        </div>
    </div>
</div>
            ...
            <form id="search_from" name="search_from" class="form fv-plugins-bootstrap5 fv-plugins-framework" onsubmit="return false;">
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <select id="state_id" name="state_id" class="form-select form-select-solid form-select" aria-label="Select" data-control="select2" data-placeholder="Select">
                            <option></option>
                            @foreach($states as $state)
                                <option value="{{ $state->id }}">{{ $state->state_name }}</option>
                            @endforeach
                        </select>
                        <span class="error" id="state_id_error"></span>
                    </div>
                    <div class="col-md-4">
                        <label class="fs-6 fw-semibold mb-1 ms-1">District Name</label>
                        <input type="text" id="district_name" name="district_name" class="form-control form-control-solid" />
                        <span class="error" id="district_name_error"></span>
                    </div>
                </div>
                <hr>
                <div>
            ...
            <table id="table_content" name="table_content" class="table align-middle table-row-dashed fs-6 gy-5">
                ...
                @foreach($lists as $key => $list)
                    <tr id="row-{{$list->id}}">
                        ...
                        <td>
                            <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                                ...
                                <input type="checkbox"
                                       ...
                                       data-district-name="{{ e($list->district_name) }}"
                                       {{ $isChecked }}>
                        ...
                        <td>
                            <a href="<?php echo url('district/add/'.$list->id);?>" class="btn btn-icon btn-active-light-info w-30px h-30px me-2" title="Edit">
                                <i class="ki-outline ki-pencil text-info fs-3"></i>
                            </a>
                            <a href="<?php echo url('district/view/'.$list->id);?>" class="btn btn-icon btn-active-light-primary w-30px h-30px me-2" title="View">
                                <i class="ki-outline ki-eye text-primary fs-3"></i>
                            </a>
                            <button type="button"
                                    class="btn btn-icon btn-active-light-danger w-30px h-30px me-2 btn-delete"
                                    data-id="{{ $list->id }}"
                                    data-name="{{ e($list->district_name) }}"
                                    title="Delete">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.status-toggle').forEach(checkbox => {
        checkbox.addEventListener('change', function(e) {
            ...
            if (confirm(`Are you sure you want to change the status of "${districtName}" to "${statusText}"?`)) {
                this.disabled = true;
                
                // Get CSRF token
                const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
                
                fetch(`/district/status/${currentStatus}/${districtId}`, {
                    method: 'GET',
                    headers: {
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', function() {
            ...
            const deleteUrl = `/district/delete/${currentDeleteId}`;
    ...
    function loadTableData(start, limit) {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                updateShowingText(data.message || `Showing ${start + 1} to ${Math.min(start + data.lists.length, data.total_count)} of ${data.total_count} records.`);
            } else {
                updateTable([]);
        ...
        lists.forEach((list, index) => {
            ...
            html += `
                <tr id="row-${list.id}">
                    ...
                    <td>${escapeHtml(list.district_name)}</td>
                    <td>
                        <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                            <input type="checkbox"
                                   id="status_${list.id}"
                                   ...
                                   data-district-name="${escapeHtml(list.district_name)}"
                                   ${isChecked}>
                            <span class="form-check-label fw-semibold ${statusClass}" for="status_${list.id}">
                                ${statusText}
                            </span>
                    ...
                    <td>
                        <a href="/district/add/${list.id}" class="btn btn-icon btn-active-light-info w-30px h-30px me-2" title="Edit">
                            <i class="ki-outline ki-pencil text-info fs-3"></i>
                        </a>
                        <a href="/district/view/${list.id}" class="btn btn-icon btn-active-light-primary w-30px h-30px me-2" title="View">
                            <i class="ki-outline ki-eye text-primary fs-3"></i>
                        </a>
                        <button type="button"
                                class="btn btn-icon btn-active-light-danger w-30px h-30px me-2 btn-delete"
                                data-id="${list.id}"
                                data-name="${escapeHtml(list.district_name)}"
                                title="Delete">
                            <i class="ki-outline ki-trash text-danger fs-3"></i>
                        </button>
                    </td>
