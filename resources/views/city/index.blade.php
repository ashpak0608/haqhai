    ...
    <div id="kt_app_toolbar_container" class="container-fluid d-flex align-items-stretch">
        <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="<?php echo url('city/add/');?>" class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold"><i class="ki-outline ki-plus fs-2"></i>Add</a>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="d-row row-wrap row-sm-nowrap pb-4">
            <form id="search_from" name="search_from" class="form fv-plugins-bootstrap5 fv-plugins-framework" onsubmit="return false;">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <label class="fs-6 fw-semibold mb-1 ms-1">City Name</label>
                        <input type="text" id="city_name" name="city_name" class="form-control form-control-solid" />
                        <span class="error" id="city_name_error"></span>
                    </div>
                </div>
                <hr>
                <div>
            ...
            <table id="table_content" name="table_content" class="table align-middle table-row-dashed fs-6 gy-5">
                ...
                @foreach($lists as $key => $list)
                    <tr id="row-{{$list->id}}">
                        ...
                        <td>
                            <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                                ...
                                <input type="checkbox"
                                       ...
                                       data-city-name="{{ e($list->city_name) }}"
                                       {{ $isChecked }}>
                        ...
                        <td>
                            <a href="<?php echo url('city/add/'.$list->id);?>" class="btn btn-icon btn-active-light-info w-30px h-30px me-2" title="Edit">
                                <i class="ki-outline ki-pencil text-info fs-3"></i>
                            </a>
                            <a href="<?php echo url('city/view/'.$list->id);?>" class="btn btn-icon btn-active-light-primary w-30px h-30px me-2" title="View">
                                <i class="ki-outline ki-eye text-primary fs-3"></i>
                            </a>
                            <button type="button"
                                    class="btn btn-icon btn-active-light-danger w-30px h-30px me-2 btn-delete"
                                    data-id="{{ $list->id }}"
                                    data-name="{{ e($list->city_name) }}"
                                    title="Delete">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.status-toggle').forEach(checkbox => {
        checkbox.addEventListener('change', function(e) {
            ...
            if (confirm(`Are you sure you want to change the status of "${cityName}" to "${statusText}"?`)) {
                this.disabled = true;
                
                // Get CSRF token
                const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
                
                fetch(`/city/status/${currentStatus}/${cityId}`, {
                    method: 'GET',
                    headers: {
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', function() {
            ...
            const deleteUrl = `/city/delete/${currentDeleteId}`;
        ...
        lists.forEach((list, index) => {
            ...
            html += `
                <tr id="row-${list.id}">
                    ...
                    <td>
                        <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                            <input type="checkbox"
                                   ...
                                   data-city-name="${escapeHtml(list.city_name)}"
                                   ${isChecked}>
                            <span class="form-check-label fw-semibold ${statusClass}" for="status_${list.id}">
                                ${statusText}
                            </span>
                    ...
                    <td>
                        <a href="/city/add/${list.id}" class="btn btn-icon btn-active-light-info w-30px h-30px me-2" title="Edit">
                            <i class="ki-outline ki-pencil text-info fs-3"></i>
                        </a>
                        <a href="/city/view/${list.id}" class="btn btn-icon btn-active-light-primary w-30px h-30px me-2" title="View">
                            <i class="ki-outline ki-eye text-primary fs-3"></i>
                        </a>
                        <button type="button"
                                class="btn btn-icon btn-active-light-danger w-30px h-30px me-2 btn-delete"
                                data-id="${list.id}"
                                data-name="${escapeHtml(list.city_name)}"
                                title="Delete">
                            <i class="ki-outline ki-trash text-danger fs-3"></i>
                        </button>
                    </td>
