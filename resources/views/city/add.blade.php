    ...
    <div id="kt_app_toolbar_container" class="container-fluid d-flex align-items-stretch">
        <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="<?php echo url('city');?>" class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold"><i class="ki-outline ki-left fs-2"></i>Back</a>
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-4">
    <div class="card-body pt-4 pb-0">
        <div class="d-row row-wrap row-sm-nowrap pb-4">
            <form id="city_form" name="city_form" method="post" class="form fv-plugins-bootstrap5 fv-plugins-framework">
            @csrf
            <input type="hidden" id="id" name="id" value="{{isset($singleData['id']) ? $singleData['id'] : ''}}"/>
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <select id="district_id" name="district_id" class="form-select form-select-solid form-select" aria-label="Select" data-control="select2" data-placeholder="Select" required>
                            ...
                            @foreach($districts as $district)
                                <option value="{{ $district->id }}" {{isset($singleData['district_id']) && $singleData['district_id'] == $district->id ? 'selected' : ''}}>
                    ...
                    <div class="col-md-4">
                        <label class="required fs-6 fw-semibold mb-1 ms-1">City Name</label>
                        <input type="text" id="city_name" name="city_name" class="form-control form-control-solid" value="{{isset($singleData['city_name']) ? $singleData['city_name'] : ''}}" />
                    </div>
                </div>
                <hr>
                <div>
