            ...
            <table id="table_content" name="table_content" class="table align-middle table-row-dashed fs-6 gy-5">
                ...
                @foreach($lists as $key => $list)
                    <tr id="row-{{$list->id}}">
                        ...
                        <td>
                            <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                                ...
                                <input type="checkbox"
                                       ...
                                       data-area-name="{{ e($list->area_name) }}"
                                       {{ $isChecked }}>
                        ...
                        <td>
                            <a href="<?php echo url('areas/add/'.$list->id);?>" class="btn btn-icon btn-active-light-info w-30px h-30px me-2" title="Edit">
                                <i class="ki-outline ki-pencil text-info fs-3"></i>
                            </a>
                            <a href="<?php echo url('areas/view/'.$list->id);?>" class="btn btn-icon btn-active-light-primary w-30px h-30px me-2" title="View">
                                <i class="ki-outline ki-eye text-primary fs-3"></i>
                            </a>
                            <button type="button"
                                    class="btn btn-icon btn-active-light-danger w-30px h-30px me-2 btn-delete"
                                    data-id="{{ $list->id }}"
                                    data-name="{{ e($list->area_name) }}"
                                    title="Delete">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.status-toggle').forEach(checkbox => {
        checkbox.addEventListener('change', function(e) {
            ...
            if (confirm(`Are you sure you want to change the status of "${areaName}" to "${statusText}"?`)) {
                this.disabled = true;
                
                // Get CSRF token
                const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
                
                fetch(`/areas/status/${currentStatus}/${areaId}`, {
                    method: 'GET',
                    headers: {
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', function() {
            ...
            const deleteUrl = `/areas/delete/${currentDeleteId}`;
    ...
    function updateTable(lists) {
        ...
        lists.forEach((list, index) => {
            ...
            html += `
                <tr id="row-${list.id}">
                    <td>${serialNo}</td>
                    <td>${escapeHtml(list.area_name)}</td>
                    ...
                    <td>
                        <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                            <input type="checkbox"
                                   ...
                                   data-area-name="${escapeHtml(list.area_name)}"
                                   ${isChecked}>
                            <span class="form-check-label fw-semibold ${statusClass}" for="status_${list.id}">
                                ${statusText}
                            </span>
                    ...
                    <td>
                        <a href="/areas/add/${list.id}" class="btn btn-icon btn-active-light-info w-30px h-30px me-2" title="Edit">
                            <i class="ki-outline ki-pencil text-info fs-3"></i>
                        </a>
                        <a href="/areas/view/${list.id}" class="btn btn-icon btn-active-light-primary w-30px h-30px me-2" title="View">
                            <i class="ki-outline ki-eye text-primary fs-3"></i>
                        </a>
                        <button type="button"
                                class="btn btn-icon btn-active-light-danger w-30px h-30px me-2 btn-delete"
                                data-id="${list.id}"
                                data-name="${escapeHtml(list.area_name)}"
                                title="Delete">
                            <i class="ki-outline ki-trash text-danger fs-3"></i>
                        </button>
                    </td>
