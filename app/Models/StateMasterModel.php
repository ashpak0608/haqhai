...
class StateMasterModel extends Model
{
    ...
    public function saveData($post) {
        ...
        $finalData = new StateMasterModel;
        foreach ($post as $k => $v) {
        ...
        if ($id == 0) {
            ...
            $finalData->save();
            $id = $finalData->id;
            return array('id' => $id, 'status' => 'success', 'message' => "State Master Data saved!");
        } else {
            if ($this->getSingleData($id)) {
                ...
                $finalData->id = $id;
                $finalData->save();
                return array('id' => $id, 'status' => 'success', 'message' => "State Master Data updated!");
            } else {
                return false;
            }
        }
