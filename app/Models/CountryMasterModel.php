...
class CountryMasterModel extends Model
{
    use HasFactory;
    ...
    protected $fillable = [
        'id', 'country_name','status', 'created_by', 'created_at', 'updated_by', 'updated_at'];

    public function getSaveData() {
        return array('id', 'country_name','status', 'created_by', 'created_at', 'updated_by', 'updated_at');
    }

    public function saveData($post) {
        $saveFields = $this->getSaveData();
        $finalData = new CountryMasterModel;
        foreach ($post as $k => $v) {
        ...
        if ($id == 0) {
            ...
            $finalData->save();
            $id = $finalData->id;
            return array('id' => $id, 'status' => 'success', 'message' => "Country Data saved!");
        } else {
            if ($this->getSingleData($id)) {
                ...
                $finalData->id = $id;
                $finalData->save();
                return array('id' => $id, 'status' => 'success', 'message' => "Country Data updated!");
            } else {
                return false;
            }
        }
    ...
    static function getAllCountryDetails($param = []){
       $query = DB::table('country_master as c');
       ...
       $query->select(DB::raw("
        ...
        if(isset($param['country_name']) && !empty($param['country_name'])){
            $query->where('c.country_name','like','%'.$param['country_name'].'%'); 
        }
        $total_count = $query->count();
        if(isset($param['limit']) && isset($param['start'])){
            $query->limit($param['limit'])->offset($param['start']);
        }
        $query->orderBy('c.country_name','asc');
        $result = $query->get();
        if($total_count > 0){
