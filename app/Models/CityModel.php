...
class CityModel extends Model
{
    ...
    protected $fillable = [
        'id','district_id', 'city_name', 'status', 'created_by', 'created_at', 'updated_by', 'updated_at'
    ];
    ...
    public function saveData($post) {
        ...
        if ($id == 0) {
            ...
            $finalData->save();
            $id = $finalData->id;
            return array('id' => $id, 'status' => 'success', 'message' => "City Data saved!");
        } else {
            if ($this->getSingleData($id)) {
                ...
                $finalData->id = $id;
                $finalData->save();
                return array('id' => $id, 'status' => 'success', 'message' => "City Data updated!");
            } else {
                return false;
            }
        }
