...
class LocationController extends Controller {
    ...
    function getFiltering(Request $request) {
        try{
            ...
            $param['location_name'] = $request->location_name ?? null;
    ...
    function save(Request $request) {
        try{
            \Log::info('Location Save Request:', $request->all());
            ...
            // Check if location exists excluding soft deleted records
            $existingLocation = DB::table($this->table)
            ...
            \Log::info('Existing Location Check:', ['exists' => !is_null($existingLocation)]);
            ...
            if ($existingLocation) {
                ...
                \Log::warning('Location exists:', $returnData);
            ...
            if (isset($returnData['status']) && $returnData['status'] == 'success') {
                return redirect()->route('location.index')->with('status', 'success')->with('message', $returnData['message']);
            } else {
                return redirect()->back()->withInput()->with('status', 'warning')->with('message', $returnData['message'] ?? 'Something went wrong');
            }
        }
        catch(\Throwable $e){
            \Log::error('Location Save Error:', [
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            $objModel = new LocationMasterModel();
            ...
            \Log::info('Deleting location:', ['id' => $id, 'location_name' => $row['location_name']]);

            // Perform soft delete: set deleted_at and deleted_by
            $deletedBy = Session::get('id') ?? null;
            $result = DB::table('locations')->where('id', $id)->update([
            ...
            $resp = ['status' => 'success', 'message' => 'Location deleted successfully', 'id' => $id];
            ...
            return redirect()->route('location.index')->with('status', 'success')->with('message', $resp['message']);
        } catch (\Throwable $e) {
