...
class CityController extends Controller {
    ...
    function index() {
       try{
            ...
            // Get only non-deleted districts
    ...
    function getFiltering(Request $request) {
        try{
            ...
            $param['district_id'] = $request->district_id ?? null;
            $param['city_name'] = $request->city_name ?? null;
    ...
    function save(Request $request) {
        try{
            \Log::info('City Save Request:', $request->all());
            ...
            // Check if city exists excluding soft deleted records
            $existingCity = DB::table($this->table)
            ...
            \Log::info('Existing City Check:', ['exists' => !is_null($existingCity)]);
            ...
            if ($existingCity) {
                ...
                \Log::warning('City exists:', $returnData);
            ...
            if (isset($returnData['status']) && $returnData['status'] == 'success') {
                return redirect()->route('city.index')->with('status', 'success')->with('message', $returnData['message']);
            } else {
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            \Log::info('Deleting city:', ['id' => $id, 'city_name' => $row['city_name']]);

            // Perform soft delete: set deleted_at and deleted_by
            $deletedBy = Session::get('id') ?? null;
            $result = DB::table('cities')->where('id', $id)->update([
            ...
            $resp = ['status' => 'success', 'message' => 'City deleted successfully', 'id' => $id];
            ...
            return redirect()->route('city.index')->with('status', 'success')->with('message', $resp['message']);
        } catch (\Throwable $e) {
